...
@login_required
def index():
    global known_face_encodings, user_name
    # Load a sample picture and learn how to recognize it.
    if current_user.foto != 'default.png':
        # ini harus dibuat per image sendiri2
        # user_image = face_recognition.load_image_file(f"app/static/img/{current_user.foto}")
        # user_face_encoding = face_recognition.face_encodings(user_image)[0]

        # Load a sample picture and learn how to recognize it.
        theis_image = face_recognition.load_image_file("app/static/img/458ca24718141c17.png")
        theis_face_encoding = face_recognition.face_encodings(theis_image)[0]

        # Load a second sample picture and learn how to recognize it.
        obama_image = face_recognition.load_image_file("app/static/img/3acc58eb4ceaaeea.jpg")
        obama_face_encoding = face_recognition.face_encodings(obama_image)[0]
        known_face_encodings = [theis_face_encoding, obama_face_encoding]
        user_name = [row.username for row in User.query.all()]
    else:
        pass
